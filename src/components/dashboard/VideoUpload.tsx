...
import { Upload, Trash2, Check, Video, AlertTriangle, Scissors } from 'lucide-react';
...
const VideoUpload = ({
  ...
  const [videoForTrimming, setVideoForTrimming] = useState<File | null>(null);
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement> | React.DragEvent<HTMLDivElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      // Check if a file needs trimming (first check duration)
      if (fileArray.length === 1) {
        try {
          const duration = await getMediaDuration(fileArray[0]);
          // If duration is outside the allowed range (50-100 seconds), show trimmer
          if (duration < 50 || duration > 100) {
            setVideoForTrimming(fileArray[0]);
            return;
          }
        } catch (error) {
      ...
      await processVideoFiles(fileArray);
    }
  };
  ...
  const uploadVideoFile = async (file: File, duration: number) => {
    setIsUploading(true);
  ...
  const handleTrimmedVideoSave = async (trimmedBlob: Blob, duration: number) => {
    if (!videoForTrimming) return;
    
    // Create a File object from the Blob
    const trimmedFile = new File(
      [trimmedBlob], 
      videoForTrimming.name, 
      { type: 'video/webm' }
    );
    
    // Upload the trimmed file
    await uploadVideoFile(trimmedFile, duration);
    
    // Reset the trimming state
    setVideoForTrimming(null);
  };
  ...
  const handleTrimVideo = (video: UploadedFile) => {
    // Create a File object from the video URL for trimming
    fetch(video.url)
      .then(response => response.blob())
      .then(blob => {
        const file = new File([blob], video.name, { type: video.type });
        setVideoForTrimming(file);
      })
      .catch(error => {
        console.error('Error fetching video for trimming:', error);
        toast({
          title: "Error",
          description: "Failed to prepare video for trimming.",
          variant: "destructive"
        });
      });
  };

  return <section className="animate-fade-in relative">
      {isUploading && <LoadingOverlay message="Uploading video..." />}
      ...
      {videoForTrimming ? (
        <VideoTrimmer 
          videoFile={videoForTrimming}
          onSave={handleTrimmedVideoSave}
          onCancel={handleCancelTrimming}
        />
      ) : (
        <>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex">
                        <Button 
                          ...
                          className="p-1.5 rounded-full mr-1 hover:bg-secondary-foreground/10 transition-colors"
                          variant="ghost"
                          size="icon"
                          title="Trim video"
                        >
